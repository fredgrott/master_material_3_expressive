...
// ignore_for_file: avoid_redundant_argument_values, non_constant_identifier_names, use_build_context_synchronously, unnecessary_async, deprecated_member_use, parameter_assignments
...
const String materialSymbolsIconsSourceFontVersionNumber = '2.872'; // must update for each new font update
const String materialSymbolsIconsSourceReleaseDate = 'Sept. 4, 2025'; // must update for each new font update
...
void main() {
  ...
  */
  MaterialSymbolsBase.setOutlinedVariationDefaults(color: outlinedColor, fill: 0.0);
  MaterialSymbolsBase.setRoundedVariationDefaults(color: roundedColor, fill: 0.0);
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final List<double> _weightValues = [100.0, 200.0, 300.0, 400.0, 500.0, 600.0, 700.0];
  ...
  void grabInitialStateFromUrl() {
    ...
    if (needToRebuildQueryParams) {
      ...
      Future.delayed(const Duration(milliseconds: 150), setQueryParametersToMatchState);
    }
  }
  ...
  TextSpan buildSummaryWidgetOfStyleAndVariation() {
    return TextSpan(
      ...
      children: const [WidgetSpan(child: Icon(Symbols.settings, color: Colors.blue, size: 18))],
    );
  }
        ...
        Clipboard.setData(ClipboardData(text: exampleIconCode)).then((_) {
          if (context.mounted) {
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(SnackBar(content: Text('Copied "$exampleIconCode" to the clipboard.')));
          }
        });
      },
      getIconCodeStringForCurrentSettings(),
      //maxLines: 4,
      style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontFamily: 'Courier New', fontSize: 10),
    );
  }
          ...
          RichText(
            text: TextSpan(
              ...
              recognizer: TapGestureRecognizer()
                ..onTap = () async {
                  launchUrl(Uri.parse('https://github.com/google/material-design-icons/tree/master/variablefont'));
                },
            ),
          ),
        SizedBox.square(
          ...
          child: IconButton(
            color: Colors.blue,
            onPressed: () {
              launchUrl(Uri.parse('https://pub.dev/packages/material_symbols_icons'));
            },
  ...
  List<Widget> buildFlexibleOptionsCustomizationPanel(BuildContext context) {
    ...
    final bool sizeDictatesUsingExpansionPanel = willSplitRows || (size.height < 500);
    final bool hardCodedUseExpansionPanel = size.height < 2600; // basically let's always use expanded panel
    ...
    final mainAxisAlignment = willSplitRows ? MainAxisAlignment.center : MainAxisAlignment.center;
    final controlPanelItems = <Widget>[
      ...Splittable.flexibleRow(
        ...
        children: <Widget>[
          const Text('Style :', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16.0)),
          Radio<FontListType>(
            value: FontListType.outlined,
            groupValue: _fontListType,
            onChanged: _onFontListTypeChange,
          ),
          const Text('Outlined', style: TextStyle(fontSize: 16.0, color: outlinedColor)),
          Radio<FontListType>(value: FontListType.rounded, groupValue: _fontListType, onChanged: _onFontListTypeChange),
          const Text('Rounded', style: TextStyle(fontSize: 16.0, color: roundedColor)),
          Radio<FontListType>(value: FontListType.sharp, groupValue: _fontListType, onChanged: _onFontListTypeChange),
          const Text('Sharp', style: TextStyle(fontSize: 16.0, color: sharpColor)),
          Radio<FontListType>(
      ...
      ...Splittable.flexibleRow(
        ...
        mainAxisAlignment: mainAxisAlignment,
        children: <Widget>[
          const Text('Icon Display:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16.0)),
          Radio<int>(
            value: 0,
      ...
      ...Splittable.flexibleRow(
        ...
        mainAxisAlignment: mainAxisAlignment,
        children: <Widget>[
          const Text('Two Tone Icon Color 1:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16.0)),
          ...
          GestureDetector(
                ...
                builder: (context) {
                  return AlertDialog(
                    ...
                    actions: [
                      ...
                      TextButton(child: const Text('Select'), onPressed: () => Navigator.of(context).pop(pickedColor)),
                    ],
                  );
                },
              ).then((picked) {
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.zero, //.circular(16),
              ),
            ),
          ),
          const SizedBox(width: 10),
          ...
          const Text('Two Tone Icon Color 2:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16.0)),
          const SizedBox(width: 10),
          GestureDetector(
            onTap: () async {
      ...
      ...Splittable.flexibleRow(
        ...
        mainAxisAlignment: MainAxisAlignment.spaceAround,
        children: <Widget>[
          const Text('Customize Variation Settings:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16.0)),
          SizedBox.square(
            dimension: 40,
            child: IconButton(
              onPressed: () {
    ...
    final controlPanelColumn = Column(
      ...
      children: <Widget>[...controlPanelItems],
    );
    ...
    return [
      ...
      if (!useExpandPanel) controlPanelColumn else ExpansionPanelList(
  ...
  List<int> searchIconNameList(String searchString) {
    ...
    for (int j = 0; j < renamedMaterialSymbolsMapKeys.length; j++) {
      if (renamedMaterialSymbolsMapKeys[j].toLowerCase().contains(searchString)) {
        ...
        final iconWasRenamedTo =
            renamedMaterialSymbolsMap[renamedMaterialSymbolsMapKeys[j]]!; // we know this is a valid key
        final int i = iconNameList.indexOf(iconWasRenamedTo);
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ConstrainedBox(
                          constraints: BoxConstraints(maxWidth: screenWidth > 500 ? 400 : screenWidth * 0.8),
                                    ...
                                    child: Tooltip(
                                      ...
                                      child: Column(
                                        children: [
                                          ...
                                          else if (hoveredOverIconData == iconDataForIndex)
                                            VariedIcon.varied(
                                              iconDataForIndex,
                                    ...
                                    children: [
                                      ...
                                      SizedBox.square(
                                        ...
                                        child: IconButton.outlined(
                                          ...
                                          onPressed: () {
                                            launchUrl(
                                              Uri.parse('https://fonts.google.com/icons?icon.set=Material+Symbols'),
                                            );
                                          },
                                          icon: const Icon(Symbols.open_in_new),
                                          style: IconButton.styleFrom(
...
class IconSearchStringInput extends StatefulWidget {
  const IconSearchStringInput({super.key, this.initialSearchText = '', required this.onSearchTextChanged});
