...
//
// Originally created by Kostia Sokolovskyi
// as material shapes
// Copyright (c) 2025 Kostia Sokolovskyi
// MIT License
...
import 'package:m3e_shapes/m3e_shapes/shapes/cubic.dart';
import 'package:m3e_shapes/m3e_shapes/shapes/morph.dart';
import 'package:m3e_shapes/m3e_shapes/shapes/rounded_polygon.dart';
import 'package:m3e_shapes/m3e_shapes/shapes/utils.dart';
...
class MaterialShapeBorder extends OutlinedBorder {
  ...
  const MaterialShapeBorder._fromCubics({required List<Cubic> cubics, super.side, this.squash = 0})
    : shape = null,
      _cubics = cubics,
      assert(squash >= 0 && squash <= 1, 'squash has to be in range [0, 1]');
  ...
  MaterialShapeBorder copyWith({RoundedPolygon? shape, BorderSide? side, double? squash}) {
    if (shape != null) {
      return MaterialShapeBorder(shape: shape, side: side ?? this.side, squash: squash ?? this.squash);
    }

    final oldShape = this.shape;

    if (oldShape != null) {
      return MaterialShapeBorder(shape: oldShape, side: side ?? this.side, squash: squash ?? this.squash);
    }

    return MaterialShapeBorder._fromCubics(cubics: _cubics, side: side ?? this.side, squash: squash ?? this.squash);
  }
