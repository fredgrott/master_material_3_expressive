...
class BlurPresetFactory {
  static Map<String, dynamic> getPresetSettings(BlurPreset preset, Color? overrideColor) {
    switch (preset) {
      case BlurPreset.soft:
        return {'blur': 5.0, 'color': overrideColor ?? const Color(0x33000000), 'boxShadow': null};
      case BlurPreset.heavy:
        return {'blur': 20.0, 'color': overrideColor ?? const Color(0x66000000), 'boxShadow': null};
      case BlurPreset.subtleShadow:
        return {
          'blur': 10.0,
          'color': overrideColor ?? const Color(0x4d000000),
          'boxShadow': [const BoxShadow(color: Color(0x33000000), blurRadius: 10.0)],
        };
      case BlurPreset.frostedGlass:
        return {
          'blur': 7.0,
          'color': overrideColor ?? const Color(0x40FFFFFF),
          'boxShadow': [const BoxShadow(color: Color(0x20000000), blurRadius: 6.0, spreadRadius: 0.5)],
      ...
      case BlurPreset.floating:
        return {
          ...
          'color': overrideColor ?? const Color(0x40FFFFFF),
          'boxShadow': [
            const BoxShadow(color: Color(0x40000000), blurRadius: 20.0, spreadRadius: 2.0, offset: Offset(0, 5)),
          ],
          'elevation': 8.0,
        };
      ...
      case BlurPreset.materialDesign:
        return {
          ...
          'boxShadow': [
            const BoxShadow(color: Color(0x20000000), blurRadius: 6.0, spreadRadius: 0.0, offset: Offset(0, 3)),
          ],
          'elevation': 4.0,
        };
    }
...
class PresetBlurBox extends StatelessWidget implements BoxProperties {
  ...
  Widget build(BuildContext context) {
    ...
    final double presetBlur = presetSettings['blur'] as double;
    final Color presetColor = presetSettings['color'] as Color;
    final List<BoxShadow>? presetBoxShadow = presetSettings['boxShadow'] as List<BoxShadow>?;
    // add the others
    ...
    return BlurBox(
      ...
      boxShadow: presetBoxShadow ?? boxShadow,
