...
// ignore_for_file: avoid_multiple_declarations_per_line, avoid_redundant_argument_values
...
import 'package:glass_widget/blurbox/interfaces/box_properties.dart';
...
class GeometricPatternBlurBox extends StatelessWidget {
  ...
  const GeometricPatternBlurBox({
    ...
    this.baseBlur = 10.0,
    this.shapeCount = 20,
    this.colors = const [Color(0x80FF5252), Color(0x807B1FA2), Color(0x802196F3), Color(0x8000BFA5)],
    this.randomPlacement = true,
    this.shapeTypes = const [GeometricShapeType.circle, GeometricShapeType.rectangle, GeometricShapeType.oval],
              ...
              builder: (context, constraints) {
                ...
                return Stack(
                  children: List.generate(shapeCount, (index) {
                    ...
                    // Calculate size
                    final sizeFactor = minSizeFactor + random.nextDouble() * (maxSizeFactor - minSizeFactor);
                    ...
                    if (randomPlacement) {
                      ...
                      left = col * colWidth + random.nextDouble() * colWidth * 0.2;
                      top = row * rowHeight + random.nextDouble() * rowHeight * 0.2;
                    }

                    // Calculate blur variation
                    final blurVariation = baseBlur * (0.5 + random.nextDouble());

                    // Create the shape
                    return Positioned(
                      left: left - (size / 2),
                      top: top - (size / 2),
                      child: _buildGeometricShape(shapeType, size, color, blurVariation, random.nextDouble() * 360),
                    );
                  }),
                );
              },
  ...
  Widget _buildGeometricShape(GeometricShapeType type, double size, Color color, double blur, double rotation) {
    Widget shape;
    ...
    switch (type) {
      case GeometricShapeType.circle:
        shape = Container(
          ...
          child: BlurBox(blur: blur, color: color, padding: EdgeInsets.zero, child: const SizedBox.expand()),
        );
      ...
      case GeometricShapeType.rectangle:
        shape = Container(
          width: size,
          height: size,
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(size * 0.1), color: Colors.transparent),
          child: BlurBox(
            blur: blur,
            color: color,
...
class _StaticOrganicPattern extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      ...
      child: BlurBox(blur: backgrundBlur, child: const SizedBox.expand()),
    );
  }
}
...
class _AnimatedOrganicPattern extends StatefulWidget {
  ...
  @override
  State<_AnimatedOrganicPattern> createState() => _AnimatedOrganicPatternState();
}

class _AnimatedOrganicPatternState extends State<_AnimatedOrganicPattern> with SingleTickerProviderStateMixin {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return CustomPaint(
          ...
          child: BlurBox(blur: widget.backgrundBlur, child: const SizedBox.expand()),
        );
      },
    );
  }
...
class _OrganicPatternPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < complexity; i++) {
      ...
      for (int j = 0; j < points; j++) {
        ...
        final radiusVariation = radius * (1 - pctVariation + random.nextDouble() * pctVariation * 2);
      ...
      for (int j = 0; j < controlPoints.length; j++) {
        ...
        path.cubicTo(ctrl1.dx, ctrl1.dy, ctrl2.dx, ctrl2.dy, nextPoint.dx, nextPoint.dy);
      }
  ...
  bool shouldRepaint(_OrganicPatternPainter oldDelegate) {
    return oldDelegate.phase != phase || oldDelegate.blur != blur || oldDelegate.color != color;
  }
}
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: Container(
        ...
        child: ClipRRect(
          borderRadius: borderRadius,
          child: animated && rotationDuration != null
...
class _AnimatedKaleidoscopeState extends State<_AnimatedKaleidoscope> with SingleTickerProviderStateMixin {
  late AnimationController _controller;
            ...
            return Stack(
              children: [
                ...
                ...List.generate(widget.segments, (index) {
                  ...
                  final rotationAngle = baseAngle + (_controller.value * 2 * pi);
    ...
    return ClipPath(
      ...
      child: Container(
        ...
        child: BlurBox(
          ...
          padding: EdgeInsets.zero,
          child: CustomPaint(
            painter: _KaleidoscopePatternPainter(color: color, patternType: patternType),
            child: const SizedBox.expand(),
          ),
        ),
      ),
...
class _SegmentClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.arcTo(Rect.fromCircle(center: center, radius: size.width), 0, angle, false);
...
class _KaleidoscopePatternPainter extends CustomPainter {
  ...
  _KaleidoscopePatternPainter({required this.color, this.patternType = KaleidoscopePatternType.basic});
  ...
  void _paintTrianglesPattern(Canvas canvas, Size size, Paint strokePaint, Paint fillPaint, Random random) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      final rotatedPath = path.transform(
        Float64List.fromList([matrix[0], matrix[1], 0, 0, matrix[4], matrix[5], 0, 0, 0, 0, 1, 0, x, y, 0, 1]),
      );
