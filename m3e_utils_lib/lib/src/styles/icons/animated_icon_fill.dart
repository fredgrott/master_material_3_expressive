...
/// While the Material Design 3 spec has icon animated fills for
/// navigation icons; the Material Symbols variable font has not made
/// it to the SDK as of yet. Thus, we need to add an animated icon
/// fill helper.
///
/// Eventually this should be changed into using Motor Motion Controllers and
/// Springs instead of the Flutter legacy curves.
library;
...
typedef AnimatedIconFillBuilder = Widget Function(BuildContext context, double fill);
...
class _AnimatedIconFillState extends State<AnimatedIconFill> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    animation = CurvedAnimation(parent: controller, curve: curve, reverseCurve: widget.reverseCurve ?? curve.flipped);
  }
  ...
  Widget build(BuildContext context) =>
      AnimatedBuilder(animation: animation, builder: (context, _) => widget.builder(context, animation.value));
}
